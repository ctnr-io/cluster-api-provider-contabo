...
test-e2e: setup-test-e2e manifests generate fmt vet clusterctl cilium ## Run the e2e tests. Expected an isolated environment using Kind.
	KIND=$(KIND) KIND_CLUSTER=$(KIND_CLUSTER) CLUSTERCTL=$(CLUSTERCTL) go test -tags=e2e ./test/e2e/ -v -ginkgo.v -timeout 15m
# 	cleanup-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name cluster-api-provider-contabo-builder
	$(CONTAINER_TOOL) buildx use cluster-api-provider-contabo-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm cluster-api-provider-contabo-builder
	rm Dockerfile.cross
...
$(CLUSTERCTL): $(LOCALBIN)
	@[ -f "$(CLUSTERCTL)-$(CLUSTERCTL_VERSION)" ] && [ "$$(readlink -- "$(CLUSTERCTL)" 2>/dev/null)" = "$(CLUSTERCTL)-$(CLUSTERCTL_VERSION)" ] || { \
		set -e; \
